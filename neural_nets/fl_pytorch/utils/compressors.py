...
class Compressor:
    ...
    def fullName(self):
        ...
        if self.compressorType == CompressorType.STANDARD_DITHERING_FP32: return f"Standard Dithering for fp32[s={self.s}]"
        ...
        if self.compressorType == CompressorType.TOPK_COMPRESSOR: return f"Top [K={self.K},D={self.D}]"
        if self.compressorType == CompressorType.RANK_K_COMPRESSOR: return f"Rank [K={self.K},D={self.D}]"
    ...
    def makeLazyCompressor(self, P):
        """ Make lazy compressor which with probility 'p' returns x/p and w.p. '1-p' return 0 """
    ...
    def makeRandKCompressor(self, D, K):
        ...
        # E[|C(x)|^2]=(d*d)/(k*k) * E[sum(zi*ei*xi)^2)] = (d*d)/(k*k) * k/d *|x|^2 = d/k * (x^2) = (w + 1) (x^2) => w = d/k-1
    ...
    def generateCompressPattern(self, rndgen, device, clientId, H):
        ...
        if self.compressorType == CompressorType.IDENTICAL:
            ...
            self.S = torch.from_numpy(rndgen.choice(self.D, self.K, replace=False)).to(torch.long).to(device)
    ...
    def compressVector(self, x):
        ...
        if self.compressorType == CompressorType.IDENTICAL:
            ...
            # We assume that we don't need to send indicies
            ...
            if self.testp.device != x.device:
                self.testp = self.testp.to(x.device)
            ...
            # 8-bit in exponent and extra bit of sign. at hardware level it's possible to get rid of mantissa (with stochastic exp. rouding)
            ...
            if self.levelsValues.device != x.device:
                self.levelsValues = self.levelsValues.to(x.device)
            ...
            out = torch.zeros_like(x)
            _, indicies = torch.topk(x.abs(), self.K)
            out[indicies] = x[indicies]
            # Similar to RAND-K we assume that we don't need to send indicies
            self.last_need_to_send_advance = self.K
        elif self.compressorType == CompressorType.RANK_K_COMPRESSOR:
            ...
            # RANK-K only sends in principle only paramaters of dyadic expansion
...
class ProbabilisticSwitchingCompressor:
    """Probability swithcing compressor"""
    def __init__(self):
        self.p = []
        self.c = []
        self.p_sum = 0.0
    ...
    def fullName(self):
        return f"probabilistic swithcing betwen {len(self.c)} compressors"
...
def test_unbiasendess():
    ...
    for compressor in ["ident", "randk:10%", "bernulli:0.5", "natural", "qsgd:10", "nat.dithering:10:2", "std.dithering:10:2"]:
        d = 10000
